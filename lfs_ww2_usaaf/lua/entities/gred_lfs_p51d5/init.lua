...
function ENT:OnTick() -- use this instead of "think"
	...
	elseif loadout == 6 then
		...
		if self.OldLoadout != loadout then
			...
			if slct != self.OldSecondary then
				if slct == 1 then
					if istable(self.Bombs) then
						self:SetRocketCount(loadout)
						self:SetAmmoSecondary(table.Count(self.Bombs))
					else
						self:SetAmmoSecondary(0)
					end
				elseif slct == 0 then
					self:SetAmmoSecondary(self:GetRocketCount(loadout))
				end
			else
				if slct == 1 then
					if secAmmo > 2 then self:SetAmmoSecondary(2) secAmmo = 2 end
					...
					if not self.Firing then
						if secAmmo > 0 then
							for i = 1,secAmmo do
								self.HVARs[i]:SetColor(Shown)
							end
							for i = secAmmo+1,10 do
								self.HVARs[i]:SetColor(Hidden)
							end
						else
							for k,v in pairs(self.HVARs) do v:SetColor(Hidden) end
						end
					end
				end
			end
...
function ENT:OnEngineStarted()
	self:EmitSound("P51D_START")
end

function ENT:OnEngineStopped()
	self:EmitSound("P51D_STOP")
end
