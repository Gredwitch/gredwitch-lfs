...
function ENT:RunOnSpawn()
	...
	local Cockpit = ents.Create( "prop_dynamic" )
	Cockpit:SetModel("models/gredwitch/f4u_lfs/f4u_4_cockpit.mdl")
	Cockpit:SetPos(self:GetPos())
	Cockpit:SetAngles(self:GetAngles())
	Cockpit:SetMoveType(MOVETYPE_NONE)
	Cockpit:Spawn()
	Cockpit:Activate()
	Cockpit:SetNotSolid( true )
	Cockpit:DrawShadow( false )
	Cockpit:SetParent( self )
	Cockpit.DoNotDuplicate = true
	self:dOwner( Cockpit )
