...
function ENT:AnimFins()
	...
	self:ManipulateBoneAngles(self.Bones.stick, Angle( 0,-self.smRoll/2,-self.smPitch/ 4) )
	...
	self:ManipulateBoneAngles( self.Bones.rudder, Angle( self.smYaw/1.5 ) )
	self:ManipulateBoneAngles( self.Bones.pedal_l, Angle( -self.smYaw/3 ) )
	self:ManipulateBoneAngles( self.Bones.pedal_r, Angle( self.smYaw/3 ) )
	...
	self:ManipulateBoneAngles(self.Bones.compass2,Angle(0,ang.y))
	self:ManipulateBoneAngles(self.Bones.vario,Angle(0,(self:GetVelocity().z/24)+180))
	
	---------------------------
	
	self:ManipulateBoneAngles(self.Bones.rpm,Angle(0,self:GetRPM()/10)) -- RPM
	local G = Angle(0,-(speed_meters*speed_meters + 0.1)/360)
	local G2 = G*10
	self:ManipulateBoneAngles(self.Bones.g_meter,G) -- G FORCE
...
function ENT:ExhaustFX()
	...
	if self.nextEFX < CurTime() then
		...
		local Pos = {
			Vector(138.478,27.4526,86.636),
			Vector(138.516,27.247,82.423),
			Vector(138.492,21.2418,61.028),
		}
