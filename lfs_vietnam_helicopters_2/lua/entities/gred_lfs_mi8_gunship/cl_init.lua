...
function ENT:AnimFins()
	...
	local GunnerR = self:GetGunner()
	local GunnerL = self:GetGunner2()
	local GunnerB = self:GetGunner3()
	
	local DriverValid = IsValid(Driver)
	local GunnerRValid = IsValid(GunnerR)
	local GunnerLValid = IsValid(GunnerL)
	local GunnerBValid = IsValid(GunnerB)
	...
	local HasGunnerB = GunnerBValid or (!GunnerBValid and DriverValid && Driver:lfsGetInput("FREELOOK"))
	
	
	if not DriverValid and not GunnerRValid and not HasGunnerR then
		gred.ManipulateBoneAngles(self,"hold_r", Angle(0))
		gred.ManipulateBoneAngles(self,"pod_r", Angle(0,0))
	elseif HasGunnerR then
		...
		gred.ManipulateBoneAngles(self,"hold_r", Angle(0,0,Yaw))
		gred.ManipulateBoneAngles(self,"pod_r", Angle(-Pitch))
	else
		gred.ManipulateBoneAngles(self,"hold_r", Angle(0))
		gred.ManipulateBoneAngles(self,"pod_r", Angle(0,0))
	end
	...
	if not DriverValid and not GunnerLValid and not HasGunnerL then
		gred.ManipulateBoneAngles(self,"hold_l", Angle(0))
		gred.ManipulateBoneAngles(self,"pod_l", Angle(0,0))
	elseif HasGunnerL then
		...
		local Pitch = math.Clamp( -EyeAngles.p,-90,90 )
		local Yaw = math.Clamp( EyeAngles.y,-90,90 )
		gred.ManipulateBoneAngles(self,"hold_l", Angle(0,0,Yaw))
		gred.ManipulateBoneAngles(self,"pod_l", Angle(-Pitch))
	else
		gred.ManipulateBoneAngles(self,"hold_l", Angle(0))
		gred.ManipulateBoneAngles(self,"pod_l", Angle(0,0))
	end
	...
	if not DriverValid and not GunnerBValid and not HasGunnerB then
		gred.ManipulateBoneAngles(self,"hold_b", Angle(0))
		gred.ManipulateBoneAngles(self,"pod_b", Angle(0,0))
	elseif HasGunnerB then
	
		if GunnerBValid then Driver = GunnerB end
		
		local EyeAngles = self:WorldToLocalAngles( Driver:EyeAngles() )
		EyeAngles:RotateAroundAxis( EyeAngles:Up(), 180 )
		EyeAngles.y = EyeAngles.y
		
		local Pitch = math.Clamp( -EyeAngles.p,-90,180 )
		local Yaw = math.Clamp( EyeAngles.y,-90,90 )
		gred.ManipulateBoneAngles(self,"hold_b", Angle(0,0,Yaw))
		gred.ManipulateBoneAngles(self,"pod_b", Angle(0,-Pitch))
	else
		gred.ManipulateBoneAngles(self,"hold_b", Angle(0))
		gred.ManipulateBoneAngles(self,"pod_b", Angle(0,0))
	end
